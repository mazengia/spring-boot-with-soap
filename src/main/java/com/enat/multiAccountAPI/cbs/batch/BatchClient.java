...
public class BatchClient extends WebServiceGatewaySupport {
    ...
    public CREATEMJRNLBOOKFSFSRES createBatch(BatchCreate batchCreate) {
        ...
        fcubsheaderType.setOPERATION(CBSParameters.BATCH_OPERATION_NAME);
        fcubsheaderType.setUBSCOMP(UBSCOMPType.FCUBS);
        fcubsheaderType.setUSERID(makerUser);
        fcubsheaderType.setMODULEID(CBSParameters.BATCH_MODULE_ID);
        fcubsheaderType.setSOURCE(CBSParameters.BATCH_SOURCE_ID);
        fcubsheaderType.setSERVICE(CBSParameters.BATCH_SERVICE_NAME);
        fcubsheaderType.setSOURCEOPERATION(CBSParameters.BATCH_OPERATION_NAME);
        ...
        multiJrnlBookFullType.setCCY(CBSParameters.LCY_CURRENCY);
...
//        if (!isNull(batchCreate.getCostCenter())) {
//            TxnMISFullType txnMISFullType = new TxnMISFullType();
////            txnMISFullType.setCOSTCD1(batchCreate.getCostCenter());
//            txnMISFullType.setTXNMIS5(batchCreate.getCostCenter());
//            multiJrnlBookFullType.setMisdetails(txnMISFullType);
//        }
    ...
    private CREATEMJRNLBOOKFSFSRES createBatchMaster(BigDecimal creditAmount, CREATEMJRNLBOOKFSFSREQ request, CREATEMJRNLBOOKFSFSREQ.FCUBSBODY fcubsbody, MultiJrnlBookFullType multiJrnlBookFullType) {
        ...
        devwsBatchMaster.setBALANCING(CBSParameters.BALANCING);
    ...
    private MultiJrnlBookFullType.DetbsJrnlTxnDetail transactionDetail(BatchTransaction transaction) {
        ...
        detbsJrnlTxnDetail.setCCY(CBSParameters.LCY_CURRENCY);
    ...
    public AUTHORIZEDEBATCHBROWSERFSFSRES authorizeBatch(AuthorizeBatchDto authorizeDTO) {
        ...
        fcubsheaderType.setOPERATION(CBSParameters.BATCH_OPERATION_NAME_AUTHORIZE);
        fcubsheaderType.setUBSCOMP(UBSCOMPType.FCUBS);
        fcubsheaderType.setUSERID(authorizerUser);
        fcubsheaderType.setMODULEID(CBSParameters.BATCH_MODULE_ID);
        fcubsheaderType.setSOURCE(CBSParameters.BATCH_SOURCE_ID);
        fcubsheaderType.setSERVICE(CBSParameters.BATCH_SERVICE_NAME);
        fcubsheaderType.setSOURCEOPERATION(CBSParameters.BATCH_OPERATION_NAME_AUTHORIZE);
